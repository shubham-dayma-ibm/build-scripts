...
source export_rebuild_wheel.sh
source variable.sh

echo "Variable exported successfully"
...
WHEEL_SCRIPT=create_wheel_wrapper.sh
cp "$PKG_DIR_PATH$BUILD_SCRIPT" ./build-script.sh
python ./build_wheels.py "$WHEEL_SCRIPT" "$PYTHON_VERSION" "$docker_image" "build-script.sh" "$VERSION" 2>&1 | tee build_log
